      ...
      <div className="space-y-5">
        <section className="rounded-lg border border-line bg-white p-4">
          <div className="flex items-center gap-4">
            <span className="flex h-14 w-14 items-center justify-center rounded-lg bg-brand-50 text-xl font-bold text-brand-700">
        ...
        <form className="rounded-lg border border-line bg-white p-4" onSubmit={handlePassword}>
          <h2 className="font-bold text-ink">Zmiana hasła</h2>
          <p className="mt-1 text-sm text-muted">Firebase może wymagać świeżego logowania przed zmianą hasła.</p>
          {message ? <p className="mt-4 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-success">{message}</p> : null}
          {error ? <p className="mt-4 rounded-lg bg-red-50 px-3 py-2 text-sm text-danger">{error}</p> : null}
          <div className="mt-4">
